...
6. **一次授权覆盖已批准的 run，tier 由机器判定**。state 写入 `authorization: {issue, approved_at, scope, autonomous_landing: true|false, tier_grants: [{patterns: [路径 glob], max_tier, guard, approved_at}]}`。
   ...
   - 共享只读资源、密钥、认证、支付、非本 run 的资源、main/DBA 强推或删除，始终不在授权范围，任何 `tier_grants` 都不得覆盖。
...
7. **危险确认按精确规则处理**。Pi 对 lane worktree 内的 VCS 变更、提交和本地 branch 清理应直接执行；不得把 Herdr `blocked` 当常规控制流。实际出现确认 UI 时，orchestrator 必须先 `get/read`，确认屏幕命令、目标和影响完全匹配 state 中已批准 lane 的本地 worktree 操作，才 `herdr agent send-keys <name> enter`。远端删除、push/merge、共享只读资源、密钥、认证、支付、main/DBA 重写一律 `blocked`。不得盲发 `enter`；宿主强制阻止的确认不是可绕过的授权门，必须记录其原文并上报。
...
4. 预检 `HERDR_ENV=1`、`herdr status`、Pi 入口、push/forge 认证/CI 通道，确认本 pane 已启用自主模式（`printenv MANA_AUTONOMOUS` 为 `1`，否则 safe-guard 会在 run 中途弹确认），并**先算 tier**：对每条 lane 的 `target` 路径跑 `check-mana-grant-scope.py --paths <paths> --allow-path ...`。未被 `tier_grants` 覆盖的 tier B lane 在这里一次性汇总上报 CTO（一个 run 最多问一次），获批后写入 `tier_grants` 再派发。缺任一预检项现在报，别等 N 条 lane 跑完。
5. 若新增 lane、扩大文件/路径边界或改变 acceptance，回 intake 更新 Issue 后重新获得一次 CTO 授权；不偷渡范围。

## §2 dispatch

- 每条需持续会话的 lane：先从当前 orchestrator pane 用 `herdr pane split --current --direction down --cwd <worktree-path> --no-focus` 创建**下方横切**的本 run 专属 pane；不得用 `--direction right`。读取返回的 `workspace_id`、`pane_id` 后，执行 `herdr agent start <agent-name> --kind pi --pane <pane-id>`。`<agent-name>` 和 branch 必须唯一，写入 state。
- Pi brief 必须自足：目标、逐条 acceptance 命令、文件/路径边界、禁触共享只读目录、禁 push/PR/merge/关闭自身 pane、交付格式。改动落在 `tier_grants` 内时，acceptance 必须包含一条 `python3 scripts/check-mana-grant-scope.py --base origin/main --allow-key <前缀>`，让工人在自己的 worktree 内先自证键范围。不得要求 worker 自开 PR、merge 或关闭自身 pane。
...
- Pi 工人没有审批层：Pi bundle 内不存在 `approvalMode`/`autoApprove` 开关，`hasUI`/`ui.confirm` 是 OMP 扩展概念。lane 停住只可能是 prompt 未投递、上下文或 `blocked` 状态，按 §0.7 与 §3 处理。
...
- 结论：人为门只有两处——safe-guard 的确认（用 `MANA_AUTONOMOUS=1` 一次性关掉）与 `tier_grants` 覆盖不到的变更（§0.6）。不要把审批 UI 当授权门。
...
3. 发现确认 UI 时按 §0.7 精确裁决；不能自动放行的操作写入 state 并上报。
...
3. 等 CI + 项目验收命令；失败则修复、重验，不能因“已授权”跳过。
